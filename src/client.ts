...
import { RetrieveAPIInfoResponse } from './resources/top-level';
import { APIPromise } from './core/api-promise';
import {
  Delegate,
  DelegateCreateParams,
  DelegateListDelegatorsResponse,
  DelegateListParams,
  DelegateListResponse,
  DelegateListTopParams,
  DelegateListTopResponse,
  DelegateListVotesParams,
  DelegateListVotesResponse,
  DelegateProfile,
  DelegateRetrieveStatsResponse,
  DelegateUpdateParams,
  Delegates,
} from './resources/delegates';
import { Health, HealthCheckResponse } from './resources/health';
import {
  Staking,
  StakingCalculateEarlyUnstakePenaltyParams,
  StakingCalculateEarlyUnstakePenaltyResponse,
  StakingRetrieveStakeHistoryParams,
  StakingRetrieveStakeHistoryResponse,
  StakingRetrieveStakeInfoResponse,
  StakingRetrieveStatsResponse,
  StakingRetrieveTopStakersParams,
  StakingRetrieveTopStakersResponse,
  StakingRetrieveTotalResponse,
  StakingRetrieveUnstakeHistoryParams,
  StakingRetrieveUnstakeHistoryResponse,
  StakingRetrieveVotingPowerParams,
  StakingRetrieveVotingPowerResponse,
} from './resources/staking';
import {
  Analytics,
  AnalyticsRetrieveDashboardResponse,
  AnalyticsRetrieveHistoryParams,
  AnalyticsRetrieveHistoryResponse,
  AnalyticsRetrieveParticipationResponse,
} from './resources/analytics/analytics';
import {
  Grant,
  GrantGetByApplicantResponse,
  GrantGetProgramsParams,
  GrantGetProgramsResponse,
  GrantGetStatsResponse,
  GrantListParams,
  GrantListResponse,
  GrantSubmitParams,
  Grants,
} from './resources/grants/grants';
import {
  Pagination,
  ProposalGetAnalyticsResponse,
  ProposalGetStatsResponse,
  ProposalListActiveResponse,
  ProposalListCategoriesResponse,
  ProposalListParams,
  ProposalListResponse,
  ProposalRetrieveResponse,
  ProposalSummary,
  ProposalSyncResponse,
  ProposalUpdateFeaturedParams,
  Proposals,
} from './resources/proposals/proposals';
import {
  Treasury,
  TreasuryListBalancesResponse,
  TreasuryListBudgetsParams,
  TreasuryListBudgetsResponse,
  TreasuryListFlowsParams,
  TreasuryListFlowsResponse,
  TreasuryListRecipientsParams,
  TreasuryListRecipientsResponse,
  TreasuryListTransactionsParams,
  TreasuryListTransactionsResponse,
  TreasuryRetrieveStatsResponse,
} from './resources/treasury/treasury';
import { User, UserUpdateProfileParams, Users } from './resources/users/users';
...
const environments = {
  production: 'https://dao-api.paxeer.app/api',
  environment_1: 'http://localhost:3229/api',
};
type Environment = keyof typeof environments;

export interface ClientOptions {
  /**
   * JWT token obtained from wallet signature authentication
   */
   ...
   * - `production` corresponds to `https://dao-api.paxeer.app/api`
   * - `environment_1` corresponds to `http://localhost:3229/api`
   */
   ...
   *
   * Defaults to process.env['PAXEER_GOVERNANCE_SDK_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['PAXEER_GOVERNANCE_SDK_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Paxeer Governance SDK API.
 */
export class PaxeerGovernanceSDK {
  apiKey: string;
   ...
   * API Client for interfacing with the Paxeer Governance SDK API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['PAXEER_GOVERNANCE_SDK_API_KEY'] ?? undefined]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env['PAXEER_GOVERNANCE_SDK_BASE_URL'] ?? https://dao-api.paxeer.app/api] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('PAXEER_GOVERNANCE_SDK_BASE_URL'),
    apiKey = readEnv('PAXEER_GOVERNANCE_SDK_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.PaxeerGovernanceSDKError(
        "The PAXEER_GOVERNANCE_SDK_API_KEY environment variable is missing or empty; either provide it, or instantiate the PaxeerGovernanceSDK client with an apiKey option, like new PaxeerGovernanceSDK({ apiKey: 'My API Key' }).",
      );
    }
    ...
    if (baseURL && opts.environment) {
      throw new Errors.PaxeerGovernanceSDKError(
        'Ambiguous URL; The `baseURL` option (or PAXEER_GOVERNANCE_SDK_BASE_URL env var) and the `environment` option are given. If you want to use the environment you must pass baseURL: null',
      );
    }

    this.baseURL = options.baseURL || environments[options.environment || 'production'];
    this.timeout = options.timeout ?? PaxeerGovernanceSDK.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('PAXEER_GOVERNANCE_SDK_LOG'), "process.env['PAXEER_GOVERNANCE_SDK_LOG']", this) ??
      defaultLogLevel;
   ...
   * API information
   */
  retrieveAPIInfo(options?: RequestOptions): APIPromise<TopLevelAPI.RetrieveAPIInfoResponse> {
  ...
  static PaxeerGovernanceSDK = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static PaxeerGovernanceSDKError = Errors.PaxeerGovernanceSDKError;
  ...
  delegates: API.Delegates = new API.Delegates(this);
  treasury: API.Treasury = new API.Treasury(this);
  grants: API.Grants = new API.Grants(this);
  analytics: API.Analytics = new API.Analytics(this);
  users: API.Users = new API.Users(this);
}

PaxeerGovernanceSDK.Health = Health;
PaxeerGovernanceSDK.Staking = Staking;
PaxeerGovernanceSDK.Proposals = Proposals;
PaxeerGovernanceSDK.Delegates = Delegates;
PaxeerGovernanceSDK.Treasury = Treasury;
PaxeerGovernanceSDK.Grants = Grants;
PaxeerGovernanceSDK.Analytics = Analytics;
PaxeerGovernanceSDK.Users = Users;

export declare namespace PaxeerGovernanceSDK {
  export type RequestOptions = Opts.RequestOptions;

  export { type RetrieveAPIInfoResponse as RetrieveAPIInfoResponse };
  ...
  export { Users as Users, type User as User, type UserUpdateProfileParams as UserUpdateProfileParams };
}

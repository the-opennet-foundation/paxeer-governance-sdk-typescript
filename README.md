...
```sh
npm install git+ssh://git@github.com:stainless-sdks/paxeer-governance-sdk-typescript.git
```
...
const response: PaxeerGovernanceSDK.HealthCheckResponse = await client.health.check();
```
...
const response = await client.health.check().catch(async (err) => {
  if (err instanceof PaxeerGovernanceSDK.APIError) {
...
1. Via the `PAXEER_GOVERNANCE_SDK_LOG` environment variable
...
const client = new PaxeerGovernanceSDK({
  logger: logger.child({ name: 'PaxeerGovernanceSDK' }),
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/paxeer-governance-sdk-typescript/issues) with questions, bugs, or suggestions.
